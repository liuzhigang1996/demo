...
import app.cib.service.bat.TransferOverseaStMService;
...
public class TransferInOverseaStMAction extends CibAction implements Approvable {
	...
	public void uploadFileLoad() throws NTBException {
		...
		TransferOverseaStMService transferOverseaStMService = (TransferOverseaStMService) appContext
				.getBean("TransferOverseaStMService");
		CorpUser corpUser = (CorpUser) this.getUser();
		// clear unavailable db data
		transferOverseaStMService.clearUnavailableDataByCorpId(corpUser
				.getCorpId());
...
//		String fileName = this.getUploadFileName();
		...
		FileRequestFileBean fileRequestFileBean = transferOverseaStMService
		...
		try {
			...
			if (errList.size() == 0) {
				...
				// 锟叫断斤拷锟斤拷锟睫讹拷
				if (!corpUser.checkUserLimit(
						Constants.TXN_TYPE_TRANSFER_OVERSEAS, equivalentMOP)) {
					// write limit report
					...
					reportMap.put("USER_LIMIT ",
									corpUser.getUserLimit(Constants.TXN_TYPE_TRANSFER_OVERSEAS));
			...
			transferOverseaStMService.cancelUpload(fileRequest.get("BATCH_ID")
	...
	public void uploadFileConfirm() throws NTBException {
		...
		String processId = flowEngineService.startProcess(
				...
				TransferInOverseaStMAction.class, fromCurrency, debitAmount
		...
		try {
			...
			transferOverseaStMService.updateStatus(fileRequest.get("BATCH_ID")
					.toString());
			Map resultData = new HashMap();
			...
			// dataMap.put("beneficiaryBank",pojoFileRequest.getBeneficiaryBank1());
			// dataMap.put("transId",pojoFileRequest.getTransId());

			dataMap.put("remark", pojoFileRequest.getRemark());
			
			/* Add by long_zg 2015-05-22 UAT6-242 第三者轉賬operator成功頁缺少save as template begin */
			/*mailService.toApprover_Seleted(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS_1TON, userName, dataMap);*/
			mailService.toApprover_Seleted(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS_1TON, userName,corpUser.getCorpId(), dataMap);
			/* Add by long_zg 2015-05-22 UAT6-242 第三者轉賬operator成功頁缺少save as template end */
			
			
		} catch (Exception e) {
			// clear unavailable db data
			transferOverseaStMService
	...
	public boolean approve(String txnType, String id, CibAction bean)
		...
		if (txnType != null) {
			...
			transferOverseaStMService.approveTransferOverseaStM(
					pojoFileRequest, corpUser, equivalentMOP);
			//add by linrui to host after upload file
			transferOverseaStMService.toHostTransferSTM(pojoFileRequest,
					corpUser, txnType);
			//end	
			...
			List beneList_temp = transferService.listTransferOverseaStmByBatchid(pojoFileRequest.getBatchId());
			...
			/*mailService.toLastApprover_Executor(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS_1TON, corpUser.getUserId(), dataMap);*/
			mailService.toLastApprover_Executor(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS_1TON, corpUser.getUserId(),corpUser.getCorpId(), dataMap);
			/* Modify by long_zg 2015-05-22 UAT6-242 第三者轉賬operator成功頁缺少save as template begin */
	...
	public boolean reject(String txnType, String id, CibAction bean)
		...
		if (txnType != null) {
			FileRequest pojoFileRequest = requestService.viewFileRequest(id);
			transferOverseaStMService.rejectFileRequest(pojoFileRequest);
			return true;
		} else {
			return false;
		}
	...
	public String viewDetail(String txnType, String id, CibAction bean)
		...
		CorpUser corpUser = (CorpUser) bean.getUser();
		FileRequest pojoFileRequest = requestService.viewFileRequest(id);
		viewPageUrl = "/WEB-INF/pages/bat/batch_transfer/upload_file_approval_viewOverseaStM.jsp";
		// add by mxl 1208
		List recList = transferService.listTransferOverseaStmByBatchid(id);
