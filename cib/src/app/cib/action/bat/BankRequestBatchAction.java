...
public class BankRequestBatchAction extends CibAction implements Approvable {
	...
	public void uploadFile() throws NTBException {
		...
		FileRequestFileBean fileRequestFileBean = bankDraftBatchService
		...
		double normalTotalAmt = fileRequestFileBean.getNormalTotleAmt();
		...
		if (errList.size() == 0) {
			...
			pojoFileRequest.setFromCurrency(corpFromAccount.getCurrency());

			// 锟斤拷锟斤拷FromAmount
			BigDecimal toAmount = new BigDecimal(pojoFileRequest.getToAmount()
					.toString());
			BigDecimal fromAmount = exRatesService.getEquivalent(corpUser
					.getCorpId(), pojoFileRequest.getFromCurrency(),
					pojoFileRequest.getToCurrency(), null, toAmount, 2);
			pojoFileRequest.setFromAmount(new Double(fromAmount.toString()));
			...
			/*if(null != normalList && normalList.size() > 0){
				for (int i = 0; i < normalList.size(); i++) {
					...
					String draftAmount = String.valueOf(tempMap.get("DRAFT_AMOUNT"));					
					int checkPurpose = transferService.checkNeedPurpose(pojoFileRequest.getCorpId(),
							pojoFileRequest.getFromAccount(), draftAmount, null, pojoFileRequest.getFromCurrency(),
		...
		this.setUsrSessionDataValue("recList", normalList);
			...
			bankDraftBatchService.cancelUpload(fileRequest.get("BATCH_ID").toString().trim(), null, null, new File(saveFilePath + fileRequest.get("FILE_NAME").toString().trim()));
	...
	public void uploadFileConfirm() throws NTBException {
		...
		setMessage(cutOffTimeService.check("XC05", pojoFileRequest.getFromCurrency(),pojoFileRequest.getToCurrency()));
		
		// mod by chen_y for CR225, move check from input page into acknowledge page
		...
		// mod by chen_y for CR225, move check from input page into acknowledge page end
		...
		String processId = flowEngineService.startProcess(
				...
				BankRequestBatchAction.class,
				pojoFileRequest.getFromCurrency(), pojoFileRequest
						.getFromAmount().doubleValue(), pojoFileRequest
		...
		try {
			...
			bankDraftBatchService.updateStatus(pojoFileRequest.getBatchId());
			Map resultData = new HashMap();
			...
			// clear unavailable db data
			bankDraftBatchService.clearUnavailableDataByPaybatchId(fileRequest
	...
	public void uploadFileCancel() throws NTBException {
		...
		bankDraftBatchService.cancelUpload(fileRequest.get("BATCH_ID")
	...
	public boolean approve(String txnType, String id, CibAction bean)
		...
		if (txnType != null) {
			...
			bankDraftBatchService.approveBankDraft(pojoFileRequest, corpUser,
					equivalentMOP);
			return true;
		} else {
			return false;
	...
	public String viewDetail(String txnType, String id, CibAction bean)
		...
		CorpUser corpUser = (CorpUser) bean.getUser();
		FileRequest pojoFileRequest = requestService.viewFileRequest(id);
		viewPageUrl = "/WEB-INF/pages/bat/batch_request/upload_file_approval_viewBankDraft.jsp";

		//add by hjs : list detail
		List recList = requestService.listBankDraftByBatchid(id);
		// add by lw 20101016
		//modified by lzg 20190602
		/*if (null != recList && recList.size() > 0){
			for (int i = 0; i < recList.size(); i++) {
				ReqBankDraft reqBankDraft = (ReqBankDraft) recList.get(i);
