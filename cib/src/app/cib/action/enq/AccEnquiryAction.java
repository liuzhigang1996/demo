 ...
 * @author nabai
 *
 * 闁跨喓鐓幋顖涘闁跨喐鏋婚幏鐑芥晸閿熶粙鏁撻弬銈嗗闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔烘骄閺�鍏夋閹峰嘲鍨归柨鐔告灮閹风兘鏁撶粣鏍村灲閹风兘鏁撻弬銈嗗i闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔告灮閹风兘鏁撶徊鐘绘晸閺傘倖瀚归柨鐔告灮閹风兘鏁撻弬銈嗗闁跨喐婢冪喊锟�
 */
...
public class AccEnquiryAction extends CibAction {
	...
	public void listAccountSummary() throws NTBException {
		...
		currentAccountList = accountEnqureService.listSummaryByAccType(user,
		...
		// APPCODE_OVERDRAFT_ACCOUNT//modify by wcc 20180208
		/*overdraftAccountList = accountEnqureService.listSummaryByAccType(user,
		...
		savingAccountList = accountEnqureService.listSummaryByAccType(user,
		...
		/*creditAccountList = creditAccountList(user,corpId);*/
	...
	public void listCurrentAccount() throws NTBException {
		...
		List currentAccountList = accountEnqureService.listCurrentAccount(user,user.getCorpId(),CibTransClient.APPCODE_CURRENT_ACCOUNT,prePage);
	...
	public void viewCurrentAccount() throws NTBException {
		...
		Map currentAccountView = accountEnqureService.viewCurrentDetail(user,
				accountNo,from_ccy);
		...
		currentAccountView.put("callMethod", "viewCurrentAccount");
	...
	public void listSavingAccount() throws NTBException {
		...
		/*List savingAccountList = accountEnqureService.listSavingAccount(user,
				user.getCorpId());*/
		List savingAccountList = accountEnqureService.listSavingAccount(user,user.getCorpId(),CibTransClient.APPCODE_SAVING_ACCOUNT,prePage);
	...
	public void viewSavingAccount() throws NTBException {
		...
		String sortOrder = Utils.null2EmptyWithTrim(this.getParameter("sortOrder"));
		if("".equals(sortOrder)) sortOrder = "1";
		Map savingAccountView = accountEnqureService.viewSavingDetail(user,
				accountNo,from_ccy);
		...
		//add by kevin 20131120
		savingAccountView.put("callMethod", "viewSavingAccount");
		...
		/*if("".equals(savingAccountView.get("CCY_CODE_OF_AC").toString().trim())){
			savingAccountView.put("CCY_CODE_OF_AC", from_ccy);
		}
		this.setUsrSessionDataValue("accDetail", savingAccountView);
		listTransHistory();*/
...
//				period = tdDetial.get("INT_FREQUENCY").toString() + tdDetial.get("REN_PERIOD").toString();
	...
	public void viewTimeDeposit() throws NTBException {
		...
		Map currentAccountView = accountEnqureService.viewDepositDetial(user,
				accountNo);
	...
	public void listOverdraftAccount() throws NTBException {
		...
		//modify by wcc 20180209
		/*List overdraftAccountList = accountEnqureService.listOverdraftAccount(
				user, user.getCorpId());*/
		List overdraftAccountList = accountEnqureService.listOverdraftAccount(user, user.getCorpId(),CibTransClient.APPCODE_OVERDRAFT_ACCOUNT,prePage);
		//modify by wcc 20180115 闁瑰嘲鐏濋崵锟絅EXT_KEY"闁汇劌瀚伴弫顓㈠磹閿燂拷
	...
	public void viewOverdraftAccount() throws NTBException {
		...
		AccountEnqureService accountEnqureService = (AccountEnqureService) Config
				.getAppContext().getBean("AccountEnqureService");
		Map overdraftAccountView = accountEnqureService.viewOverdraftDetial(
		...
		//add by kevin 20131120
		overdraftAccountView.put("callMethod", "viewOverdraftAccount");
		if("".equals(overdraftAccountView.get("CCY_CODE_OF_AC").toString().trim())){
	...
	public void listLoanAccount() throws NTBException {
		...
		//modify by wcc 20180209
		/*List loanAccountList = accountEnqureService.listLoanAccount(user, user
				.getCorpId());*/
		List loanAccountList = accountEnqureService.listLoanAccount(user, user.getCorpId(),CibTransClient.APPCODE_LOAN_ACCOUNT,prePage);
		CorpAccountService corpAccountService = (CorpAccountService) Config
				.getAppContext().getBean("CorpAccountService");
		//modify by wcc 20180115 闁瑰嘲鐏濋崵锟絅EXT_KEY"闁汇劌瀚伴弫顓㈠磹閿燂拷
		...
		for (int i = 0; i < loanAccountList.size(); i++) {
			...
			//modify by wcc 20180211 闁哄棗鍊瑰鍌炲礂閸喐鏆堥梺鎻掝煭缁辨繄绮垫径澶夌鞍闁告艾楠搁崯锟藉绩閹呯；
			/*row.put("ACCOUNT_NAME", corpAccountService.getAccountName(row.get(
	...
	public void viewLoanAccount() throws NTBException {
		...
		Map currentAccountView = accountEnqureService.viewLoanDetail(user,
				accountNo);
	...
	public void listTransHistory() throws NTBException {
		...
		AccountEnqureService accountEnqureService = (AccountEnqureService) context
		...
		maps = this.getParameters();
		String accountNo = Utils.null2EmptyWithTrim(this.getParameter("accountNo"));
		//keyAll閻庢稒蓱閺備線骞嶉敓鑺ョ畳闂佹鍠栭敓浠嬪触閸絾宕抽柡澶堝劤濞堟垹锟藉Δ鍐惧剨濞戞搫鎷�
		String keyAll = Utils.null2EmptyWithTrim(this.getParameter("keyAllHis"));
		//閻犙冾儑缁増绋夋繝浣侯伇濡炪倗鏁稿▓鎴︽煥椤旂》鎷�
		String prePage = Utils.null2EmptyWithTrim(this.getParameter("prePageHis"));
		//閻犙冾儑缁増绋夌�ｂ晝顏卞銈囨暩濞堟垿鏌ㄩ纭锋嫹
		String nextPage = Utils.null2EmptyWithTrim(this.getParameter("nextPageHis"));
		//闁告帇鍊栭弻鍥倷閻熸澘姣婂ù婊冩缁楀倹绋夐敓濮愶拷閺夆晜蓱濡插憡绋夌�ｂ晝顏卞銈忔嫹
		String flagPage = Utils.null2EmptyWithTrim(this.getParameter("flagPageHis"));
		//闁告帇鍊栭弻鍥及椤栨碍鍎婇柡鍕靛灣椤戝洦绋夐敓濮愶拷
		if("".equals(keyAll)){
			stackPage.push(prePage);
		}else{
		...
		if (dateRange.equals("date")) {
			...
			if (range.equals("1")) {
				...
				//modify by wcc 20180123 Last 3 Months
				int todayNum = cal.get(Calendar.DATE);
				dateToStr = DateTime.formatDate(cal.getTime(), "yyyyMMdd");
				dateTo = Long.parseLong(dateToStr);
		...
		List transHistoryList = accountEnqureService
		...
		/*List accHisFromHost = accountEnqureService.listTransHistory(corpUser,
				getParameter("accountNo"));*/
//		List accHisFromHost = accountEnqureService.listTransHistory(corpUser,accountNo, dateFrom,dateTo,prePage,ccy);
		List accHisFromHost = accountEnqureService.listTransHistory(corpUser,accountNo, dateFrom,dateTo,prePage, ccy, sortOrder);
		...
		transHistoryList = accountEnqureService.filter(null,
				accHisFromHost);
		/*transHistoryList = accountEnqureService.filter(transHistoryList,
				accHisFromHost);*/
		...
		if (transHistoryList != null) {
			for (int i = 0; i < transHistoryList.size(); i++) {
				...
				.toString());*/
				//modify by wcc 2018 闁绘粍婢樺﹢顏堝箳閵夈儱缍撳☉鎿冨弨缁绘垿宕堕悙鍨暠cr_Dr_Code闁挎稑鐡�:DEBIT  C:CREDIT闁挎冻鎷�
...
//		List accHisFromHost = accountEnqureService.listTransHistory(corpUser,accountNo, dateFrom,dateTo,prePage,ccy);
	...
	public void listCreditAccount() throws NTBException {
		...
		AccountEnqureService accountEnqureService = (AccountEnqureService) Config
				.getAppContext().getBean("AccountEnqureService");
		List creditAccountList = accountEnqureService.listCreditAccount(user, user.getCorpId()) ;
		// modify by hjs
		Map row = null;
		/*int status4Count = 0;
		for (int i = 0; i < creditAccountList.size(); i++) {
			row = (Map) creditAccountList.get(i);
			String accountNo = row.get("CREDIT_CARD_NO").toString().trim() ;
			row.put("ACCOUNT_NAME", corpAccountService.getAccountName(accountNo));
//			if(row.get("ACCOUNT_STATUS").equals("4")){
//				status4Count++;
//			}
		}*/
//		if(creditAccountList.size() == status4Count){
//			creditAccountList.clear();
//		}
		Map resultData = new HashMap();
		Log.info("creditAccountList="+creditAccountList);
	...
	public void viewCreditAccount() throws NTBException {
		...
		AccountEnqureService accountEnqureService = (AccountEnqureService) Config
				.getAppContext().getBean("AccountEnqureService");
		Map creditCardView = accountEnqureService.viewCreditDetail(user, accountNo) ;
		creditCardView.put("ACCOUNT_TYPE", accService
				.getAccountType(accountNo));
		creditCardView.put("fromPage", Utils.null2EmptyWithTrim(this
				.getParameter("fromPage")));
		...
		creditCardView.put("showStatement",
				StringUtils.countMatches(statementForDetail, str)>2?"show":"notshow");
		
		creditCardView.put("callMethod", "viewCreditAccount");
		
		this.setUsrSessionDataValue("accDetail", creditCardView);
		// listTransHistory();
...
//		BigDecimal balance = (BigDecimal) accDetail.get("CARD_BALANCE");
		...
		List transHistoryList = accountEnqureService
				.listTransHistoryFromDB(condition);
		Log.info("transHistoryList="+transHistoryList);
		// 闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔告灮閹凤拷
		List accHisFromHost = accountEnqureService.listCreditTransHistory(corpUser,
				getParameter("accountNo"));
		Log.info("accHisFromHost="+accHisFromHost);
		// 闁跨喕绶濈拠褎瀚�, 缁涙盯锟� add by hjs 2006-09-30
		transHistoryList = accountEnqureService.filterCredit(transHistoryList,
				accHisFromHost);
		...
		if (transHistoryList != null) {
			for (int i = 0; i < transHistoryList.size(); i++) {
				...
				String cr_Dr_Code = (String) txnHist.get("DR_CR_CODE");
				//txnHist.put("BALANCE", balance);
				// 闁跨喐鏋婚幏鐑芥晸缁辩暴bit闁跨喐鏋婚幏绋edit
