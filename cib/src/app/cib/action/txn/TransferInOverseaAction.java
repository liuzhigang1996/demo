 ...
 * 
 */
public class TransferInOverseaAction extends CibAction implements Approvable {
...
//		setMessage(RBFactory.getInstance("app.cib.resource.common.errmsg",Constants.US).getString("warnning.transfer.DifferenceCcy"));
	...
	public void add() throws NTBException {
		...
		TransferOversea transferOversea = new TransferOversea(CibIdGenerator
		...
		String countryCode = Utils.null2Empty(this
				.getParameter("beneficiaryCountry"));
		String beneficiaryName3 = Utils.null2Empty(this
				.getParameter("beneficiaryName3"));
		String beneficiaryName4 = Utils.null2Empty(this
				.getParameter("beneficiaryName4"));

		if (("".equals(beneficiaryName3)) && ("".equals(beneficiaryName4))) {
			String checkFlag = transferService.getAddrMandatoryFlagOversea(
					countryCode, "E");//mod by linrui for mul-language
		...
		// get the other city and other bank
		// other bank
...
//		CorpAccount corpChargeAccount = corpAccountService.viewCorpAccount(transferOversea.getChargeAccount());
		...
		if (transferOversea.getTransferAmount().doubleValue() == 0) {
			...
			transAmountService.checkMinAmtOtherBanks(transferOversea
			...
			transAmountService.checkMinAmtOtherBanks(transferOversea
					.getToCurrency(), transferOversea.getTransferAmount()
					.doubleValue());
		...
		/*fromHost = transferService.toHostChargeEnquiryNew(transferOversea
				...
				.getBeneficiaryCountry(), CGD, transferOversea
				.getFromCurrency(),transferOversea.getToCurrency(),
				transferOversea.getChareBy(),
				chargeCurrency, Iban);*/
		...
		transferOversea.setHandlingAmount(new Double(chargeFromAmount));
		transferOversea.setChargeAmount(new Double(chargeToAmount));*/
		...
		transferService.toHostOverseasTrial(transferOversea, corpUser);
		//end
		//by wency 20130118
//		this.checkDuplicatedInput("O", transferOversea);
	...
	public void addConfirm() throws NTBException {
		...
		//checkCutoffTimeAndSetMsg(transferOversea);
		...
		try {
			...
			// this.convertPojo2Map(transferOversea, logMap);
			...
			dataMap.put("beneficiaryAccName", transferOversea
					.getBeneficiaryName1());
			dataMap.put("beneficiaryBank", transferOversea
					.getBeneficiaryBank1());
			dataMap.put("transId", transferOversea.getTransId());
			dataMap.put("remark", transferOversea.getRemark());

			/* Add by long_zg 2015-05-22 UAT6-242 第三者轉賬operator成功頁缺少save as template begin */
			/*mailService.toApprover_Seleted(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS, userName, dataMap);*/
			//mailService.toApprover_Seleted(Constants.TXN_SUBTYPE_TRANSFER_OVERSEAS, userName ,corpUser.getCorpId(), dataMap);
			/* Add by long_zg 2015-05-22 UAT6-242 第三者轉賬operator成功頁缺少save as template end */
			
		} catch (Exception e) {
			Log
					.error(
							"Error process confirmation of transfering in Oversea",
							e);
	...
	public void addCancel() throws NTBException {
		...
		if ("Y".equals(transferOversea.getBankFlag())) {
			...
			resultData.put("beneficiaryBank1", transferOversea
					.getBeneficiaryBank1()
	...
	public boolean approve(String txnType, String id, CibAction bean)
		...
		if (txnType != null) {
			...
			if (inputAmountFlag.equals(Constants.INPUT_AMOUNT_FLAG_FROM)) {
				...
				transferOversea.setTransferAmount(new Double(new_To_Amount_temp
						.toString()));*/
				...
				/*BigDecimal new_From_Amount_temp = exRatesService.getEquivalentFromHostRate(
						corpUser.getCorpId(),
						transferOversea.getFromCurrency(), currency, null,
						new BigDecimal(transferAmount.toString()), 2,exchangeMap);
				transferOversea.setDebitAmount(new Double(new_From_Amount_temp
						.toString()));*/
...
//				transferDate=DateTime.getDateFromStr(DateTime.formatDate(transferDate, "yyyy-MM-dd HH:mm:ss"));	
				...
				Log.info("TransferInOverseaAction.approve() transferDate="+transferDate+" currentDate="+currentDate);
				// transfer date expired
				if(currentDate.after(transferDate)){
					Log.info("TransferInOverseaAction.approve() REJECT: currentDate.after(transferDate)");
					// reject
			...
			if (!transferLimitService.checkLimitQuota(transferOversea
				...
				reportMap.put("LOCAL_TRANS_CODE", transferOversea.getTransId());
				reportMap.put("FROM_CURRENCY", transferOversea
						.getFromCurrency());
				reportMap.put("FROM_ACCOUNT", transferOversea.getFromAccount());
				reportMap.put("TO_CURRENCY", transferOversea.getToCurrency());
				reportMap.put("FROM_AMOUNT", transferOversea.getDebitAmount());
			...
			dataMap.put("transferDate", DateTime.formatDate(transferOversea
	...
	public String viewDetail(String txnType, String id, CibAction bean)
		...
		/*Map exchangeMap = exRatesService.getExchangeRateFromHost(corpUser.getCorpId(),
				transferOversea.getFromCurrency(), transferOversea
						.getToCurrency(), 8);
		String rateType = (String) exchangeMap.get("rateType");
		BigDecimal exchangeRate = new BigDecimal("0");
		BigDecimal showExchangeRate = new BigDecimal("0");
		...
		BigDecimal equivalentMOP = exRatesService.getEquivalent(corpUser
				.getCorpId(), transferOversea.getToCurrency(), "MOP",
				new BigDecimal(transferOversea.getTransferAmount().toString()),
				null, 2);
		...
		/*if(transferOversea.getExchangeRate() < 1){
			transferOversea.setExchangeRate(transferOversea.getShowExchangeRate());
	...
	private void checkCutoffTimeAndSetMsg(TransferOversea transferOversea,String language)
			throws NTBException {
		ApplicationContext appContext = Config.getAppContext();
	...
	// Jet added 2008-1-16
	/*
	 * the precedure is: 1. Validate the length of IBAN. Every country has a
	 * fixed IBAN length. 2. The first 2 characters must be a country code which
	 * is equal to the country of the beneficiary bank. 3. Checksum validation:
	 * a. Check that the total IBAN length is correct as per the country. If
	 * not, the IBAN is invalid. b. Move the four initial characters to the end
	 * of the string. c. Replace the letters in the string with digits,
	 * expanding the string as necessary, such that A=10, B=11 ... Z=35. d.
	 * Convert the string to an integer and mod-97 the entire number.
	 * 
	 * For example, PT50 0002 0123 1234 5678 9015 4 : Step 1: Check the IBAN
	 * length. The IBAN length is 25. Match with country Portugal (PT). Step 2:
	 * Move the four initial characters to the end of the string. PT50 0002 0123
	 * 1234 5678 9015 4 ----> 0002 0123 1234 5678 9015 4 PT50 Step 3: Replace
	 * the letters with digits. 0002 0123 1234 5678 9015 4 PT50 -----> 0002 0123
	 * 1234 5678 9015 4 25 29 50 Step 4: Convert the string to an integer and
	 * mod-97 000201231234567890154252950 mod 97 = 1 The remainder is 1. Valid
	 * IBAN number.
	 */
	private void checkIBANAccount(TransferOversea transferOversea)
			throws NTBException {
		ApplicationContext appContext = Config.getAppContext();
		GenericJdbcDao dao = (GenericJdbcDao) appContext
				.getBean("genericJdbcDao");
		...
		// 1. Validate the length of IBAN. Every country has a fixed IBAN
		// length.
		if (!length.equals("00")) {
			String accountLength = (new Integer(transferOversea.getToAccount()
					.length())).toString();
		...
		// 2. check the first 2 characters must be a country code which is equal
		// to the country of the beneficiary bank.
		// if (!"".equals(prefix.trim()) && prefix.length() >2 ) {
		// prefix = prefix.substring(0,2);
		String accountPrefix = transferOversea.getToAccount().substring(0, 2)
				.toUpperCase();
		String toAccount1 = accountPrefix
				+ transferOversea.getToAccount().substring(2);
		transferOversea.setToAccount(toAccount1);
		...
		if (!length.equals("00") && new Integer(length).intValue() > 4) {
			...
			// + prefixConversion(transferOversea.getToAccount().substring(0,
			// 1).toUpperCase())
			// + prefixConversion(transferOversea.getToAccount().substring(1,
			// 2).toUpperCase())
			// + transferOversea.getToAccount().substring(2, 4);

			String tempIBANString = transferOversea.getToAccount().substring(4)
					+ transferOversea.getToAccount().substring(0, 4);
			String newIBANString = "";
			for (int i = 0; i < tempIBANString.length(); i++) {
				newIBANString = newIBANString
						+ prefixConversion(tempIBANString.substring(i, i + 1)
								.toUpperCase());
			}
