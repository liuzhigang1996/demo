...
public class RcCurrencyMacau implements NTBResources {
    private static List objList = null;
    private static boolean refreshed = false;
    private NTBUser user;
    public RcCurrencyMacau() {
        if(!refreshed){
            populate();
        }
    }
...
//        String sql = " SELECT HS_CURRENCY.CCY_CODE, HS_CURRENCY_NAME.CCY_LONG_NAME FROM HS_CURRENCY,HS_CURRENCY_NAME WHERE HS_CURRENCY.CCY_CODE=HS_CURRENCY_NAME.CCY_CODE AND HS_CURRENCY.MIN_AMT_OTHER_BANKS <> 0 ORDER BY SORTING_SEQ_MACAU";
        //String sql = " SELECT c.CCY_CODE, cn.CCY_LONG_NAME FROM HS_CURRENCY c,HS_CURRENCY_NAME cn,HS_EXCHANGE_RATE er WHERE ((c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND er.TT_BUY_RATE_MOP > 0) OR (c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND c.CCY_CODE='MOP'))AND qexr_exr_typ = 'BOK' ORDER BY SORTING_SEQ_MACAU";
          //String sql = " SELECT c.CCY_CODE, cn.CCY_LONG_NAME FROM HS_CURRENCY c,HS_CURRENCY_NAME cn,HS_EXCHANGE_RATE er WHERE ((c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND er.TT_BUY_RATE_MOP > 0) OR (c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND c.CCY_CODE='MOP'))AND qexr_exr_typ = 'BOK' and c.CCY_CODE in ('MOP','HKD','USD') ORDER BY SORTING_SEQ_MACAU";
          String sql = " SELECT c.CCY_CODE, cn.CCY_LONG_NAME, cn.LOCAL_CODE FROM HS_CURRENCY c,HS_CURRENCY_NAME cn,HS_EXCHANGE_RATE er WHERE ((c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND er.TT_BUY_RATE_MOP > 0) OR (c.CCY_CODE=cn.CCY_CODE AND c.CCY_CODE=er.CCY_CODE AND c.CCY_CODE='MOP'))AND qexr_exr_typ = 'BOK' and c.CCY_CODE in ('MOP','HKD','USD','EUR')  ORDER BY SORTING_SEQ_MACAU";
        try {
