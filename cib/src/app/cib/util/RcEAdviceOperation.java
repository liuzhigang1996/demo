 ...
 * 2007-5-9
 */
public class RcEAdviceOperation implements NTBResources {
    private static List objList = null;
    private static boolean refreshed = false;

    public RcEAdviceOperation() {
        if(!refreshed){
            populate();
        }
    }
	...
	public void populate() {
        ...
        GenericJdbcDao dao = (GenericJdbcDao) appContext.getBean(
                "genericJdbcDao");
        String sql = " SELECT distinct TYPE_OF_OPERATION as OPR_KEY, TYPE_OF_OPERATION FROM HS_E_ADVICE";
        try {
